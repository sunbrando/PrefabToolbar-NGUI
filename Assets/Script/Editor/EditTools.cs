...
public class EditTools : EditorWindow {
    ...
    public static bool CopyComponent(MonoBehaviour sourceMon, MonoBehaviour destMon)
    {
        ...
        if (sourceType == destType)
        {
            ...
            int labSize = 0;
            int depth = 0;
            UIWidget widg = destMon as UIWidget;
            if (widg)
	        {
                depth = widg.depth;
	        }
            if (destType.ToString() == "UILabel")
            {
                labTxt = (destMon as UILabel).text == (sourceMon as UILabel).text ? labTxt : (destMon as UILabel).text;
                labSize = (destMon as UILabel).fontSize == (sourceMon as UILabel).fontSize ? labSize : (destMon as UILabel).fontSize;
            }
            ...
            if (labTxt != null)
            {
                (destMon as UILabel).text = labTxt;
            }
            if (labSize != 0)
            {
                (destMon as UILabel).fontSize = labSize;
            }
            if (depth != 0)
            {
                (destMon as UIWidget).depth = depth;
